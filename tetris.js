...
class TetrisGame {
    ...
    drawFloatingTexts() {
        ...
        this.floatingTexts.forEach((ft, index) => {
            ...
            ctx.fillText(ft.text, ft.x * blockSize + blockSize/2, ft.y * blockSize - ft.offset);
            ctx.restore();
        });
    }
    ...
    drawBlock(ctx, x, y, color, size = this.BLOCK_SIZE) {
        ...
        ctx.strokeRect(x * size + offset, y * size + offset, size - offset * 2, size - offset * 2);
    }
    ...
    dropPiece() {
        if (!this.movePiece(0, 1)) {
            ...
            this.spawnPiece();
            if (!cleared) {
                this.addFloatingText(this.currentPiece.x + 1, this.currentPiece.y - 1, '+2', '#cccccc');
            }
        }
    }
    ...
    clearLines() {
        ...
        if (linesCleared > 0) {
            ...
            this.dropInterval = Math.max(100, this.baseDropInterval - (this.level - 1) * 50);
            // show floating text
        ...
        const animate = () => {
            ...
            rows.forEach(y => {
                for (let x = 0; x < COLS; x++) {
                    ...
                    this.ctx.fillStyle = flashColor + Math.floor(alpha * 255).toString(16).padStart(2, '0');
    ...
    calculateScore(lines) {
        ...
        if (newCheckpoints > oldCheckpoints) {
            ...
            this.dropInterval = Math.max(100, this.baseDropInterval / speedMultiplier);
        }
    ...
    setupEventListeners() {
        ...
        // New mobile controls
        const newRotateBtn = document.getElementById('newRotateBtn');
        const newLeftBtn = document.getElementById('newLeftBtn');
        const newRightBtn = document.getElementById('newRightBtn');
        const newDropBtn = document.getElementById('newDropBtn');
        
        // Setup new controls with continuous movement
        ...
        if (newLeftBtn) {
            newLeftBtn.addEventListener('touchstart', (e) => {
                e.preventDefault();
                startTouch(() => this.movePiece(-1, 0));
            });
            newLeftBtn.addEventListener('touchend', stopTouch);
            newLeftBtn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                startTouch(() => this.movePiece(-1, 0));
            });
            newLeftBtn.addEventListener('mouseup', stopTouch);
            newLeftBtn.addEventListener('mouseleave', stopTouch);
        }
        ...
        if (newRightBtn) {
            ...
            newRightBtn.addEventListener('touchend', stopTouch);
            newRightBtn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                startTouch(() => this.movePiece(1, 0));
            });
            newRightBtn.addEventListener('mouseup', stopTouch);
            newRightBtn.addEventListener('mouseleave', stopTouch);
        }
        
        // New rotate button
        if (newRotateBtn) {
            newRotateBtn.addEventListener('touchstart', (e) => {
                e.preventDefault();
                this.rotatePiece();
            });
            newRotateBtn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                this.rotatePiece();
            });
        }
        ...
        if (newDropBtn) {
            ...
            newDropBtn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                this.dropPiece();
            });
        }
    ...
    togglePause() {
        ...
        document.getElementById('playPauseBtn').textContent = this.paused ? 'Resume' : 'Pause';
    }
    ...
    getShadowY() {
        ...
        while (!this.checkCollision(this.currentPiece, 0, shadowY - this.currentPiece.y + 1)) {
            shadowY++;
        }
        return shadowY;
    }
